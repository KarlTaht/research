...
# Import low-frequency mask from metrics package
from .metrics.curvature import make_low_freq_mask
...
def spectral_smoothness_loss(
    ...
    Unlike spectral_smoothness() in metrics.curvature, this uses softmax
    probabilities instead of hard predictions, allowing gradient flow.

    Note: This is expensive - computes forward pass on all p^2 inputs.
...
def jacobian_regularizer(
    ...
    Penalizes ||grad_x f(x)||^2 to encourage first-order smoothness.
...
class LeastActionLoss(nn.Module):
    ...
    L = L_task + lambda_routing * L_routing + lambda_spectral * L_spectral
